<?php 
	...
	if (isset($_COOKIE["idUsr"]) && isset($_COOKIE["marcaUsr"])){
    	if ($_COOKIE["idUsr"]!="" || $_COOKIE["marcaUsr"]!=""){
       		...
       		$str = "SELECT * FROM tbl_user WHERE cod_usr = :idUsr AND scookie_usr = :marca AND scookie_usr <> '' AND bativo_usr = 1 ";
	...
	if (isset($_POST['flagSubmit'])){
		...
		$valid = false;
       	...
       	$str = " SELECT * FROM tbl_user WHERE semail_usr = :email ";
		$rst = $conexao->prepare($str);
   		...
   		if ($valid){
			if (isset($_POST['flagLogged']) and $_POST['flagLogged'] == 1){
				$numero_aleatorio = mt_rand(1000000,999999999);
 				$str = "UPDATE tbl_user SET scookie_usr = :scookie_usr WHERE cod_usr = :cod_usr ";
				$rst = $conexao->prepare($str);
		...
		$(document).ready(function()
		{
			$('.form-signin').submit(function(event){
				...
				var data = {
						...
						email: email,
						senha: senha,
						flagLogged: $('#flagLogged').is(':checked') ? 1 : 0
				}
